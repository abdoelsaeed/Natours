...
const { convertIdToName } = require('./tourController');

const handelDublicateFieldsDB = async err => {
  const value = err.errmsg.match(/'([^']+)'/g)[0];
  const finalValue = value.replace(/^'|'$/g, '');
  const a = await convertIdToName(finalValue);

  const message = `Duplicate field id: ${a}  please use a different name`;
